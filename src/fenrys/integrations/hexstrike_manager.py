...
class HexStrikeManager:
    ...
    def _resolve_python(self, path: Path) -> Path:
        ...
        explicit = self.python_override.strip()
        # A generic "python3" override should not hide an installed HexStrike
        # virtualenv; dedicated environments are preferred when present.
        if explicit and Path(explicit).name not in {"python", "python3"}:
        ...
        if explicit:
            choices.append(explicit)
        choices.extend([shutil.which("python3") or "", sys.executable])
