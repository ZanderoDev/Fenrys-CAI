...
DEFAULT_AGENTS = {
    "model_tiers": {
        "heavy_reasoning": {"provider": "openrouter", "model": "minimax/minimax-m3:free"},
        "balanced": {"provider": "openrouter", "model": "minimax/minimax-m3:free"},
        "fast_cheap": {"provider": "openrouter", "model": "minimax/minimax-m3:free"},
    },
    "agents": {
...
class ConfigManager:
    ...
    def __init__(self, project_root: Path | None = None, home: Path | None = None):
        ...
        if home:
            self.user_dir = home / ".config" / "fenrys-cai"
        else:
            # Detect actual home: check if running as root
            actual_home = Path("/root") if os.getuid() == 0 else Path.home()
            config_home = Path(os.environ.get("XDG_CONFIG_HOME", actual_home / ".config"))
            self.user_dir = config_home / "fenrys-cai"
    ...
    def save(self, name: str, data: dict[str, Any], backup: bool = True) -> Path:
        ...
        directory = self.project_dir if project_path.exists() else self.user_dir
    ...
    def write_initial(self, mode: str = "NORMAL", target: str | None = None) -> None:
        ...
        for name, data in (
            ("providers.yaml", self.load("providers.yaml")),
            ("agents.yaml", self.load("agents.yaml")),
            ("policy.yaml", self.load("policy.yaml")),
            ("scope.yaml", scope),
        ):
            self.save(name, data, backup=False)
