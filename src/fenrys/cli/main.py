...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--v2", action="store_true", help="Launch the v2 streaming UI")
    parser.add_argument("--textual", action="store_true", help="Launch the legacy Textual TUI")
    ...
    tools.add_argument("tools_command", choices=["list", "status", "sync"], nargs="?", default="status")
    ...
    run = session_sub.add_parser("run"); run.add_argument("session_id"); run.add_argument("agent"); run.add_argument("objective")
    investigate = session_sub.add_parser("investigate"); investigate.add_argument("session_id"); investigate.add_argument("objective")
    report = sub.add_parser("report"); report.add_argument("--format", choices=["markdown", "json", "html"], default="markdown"); report.add_argument("--session")
...
def _open_ui(config: ConfigManager, setup: bool = False, v2: bool = False, textual: bool = False) -> None:
    if setup:
        from fenrys.ui import run_setup_wizard
        run_setup_wizard(config, reconfigure=True)
        if not config.is_configured():
            return
    if textual or v2:
        from fenrys.ui import FenrysApp, FenrysAppV2
        if v2:
            FenrysAppV2(config).run()
        else:
            FenrysApp(config).run()
        return
    from fenrys.ui import FenrysREPL
    asyncio.run(FenrysREPL(config).run())
...
def main(argv: list[str] | None = None) -> None:
    ...
    textual = getattr(args, "textual", False)
    if command is None:
        if not config.is_configured():
            _open_ui(config, setup=True, textual=textual)
        else:
            _open_ui(config, v2=v2, textual=textual)
        return
    if command == "setup":
        _open_ui(config, setup=True, textual=textual)
    elif command == "doctor":
        ...
        for agent in config.load("agents.yaml")["agents"]:
        ...
        from fenrys.gateway.server import GatewayServer
...
def handle_tools(config: ConfigManager, args) -> None:
    ...
    if args.tools_command == "list":
        ...
        console.print("Native HexStrike MCP tools are synced automatically when a session needs them.")
...
def handle_hexstrike(config: ConfigManager, args) -> None:
    ...
    if args.hexstrike_command == "start":
        pid = manager.start()
        console.print(f"[green]HexStrike server running[/green] (pid {pid}, {cfg.get('hexstrike_url', 'http://127.0.0.1:8888')})")
...
def handle_session(config: ConfigManager, args) -> None:
    ...
    try:
        ...
        elif args.session_command in {"run", "investigate"}:
            from fenrys.agents import InvestigationRuntime
            runtime = InvestigationRuntime(config, store)
            if args.session_command == "run":
                result = asyncio.run(runtime.run_task(args.session_id, args.agent, args.objective))
                console.print(f"[green]Task {result.task_id} {result.tool_calls} tool call(s)[/green] via {result.provider}/{result.model}")
                if result.blocked_reason:
                    console.print(f"[yellow]Blocked:[/yellow] {result.blocked_reason}")
                elif result.summary:
                    console.print(result.summary[:2000])
            else:
                results = asyncio.run(runtime.run_investigation(args.session_id, args.objective))
                for result in results:
                    status = "BLOCKED" if result.blocked_reason else "COMPLETED"
                    console.print(f"[green]{status}[/green] {result.task_id} {result.tool_calls} tool call(s) {result.provider}/{result.model}")
    finally:
        store.close()
